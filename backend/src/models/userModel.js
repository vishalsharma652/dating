...
async function create({ name, email = null, phone, password, role = 'user', phoneVerified = false, emailVerified = false, gender = null }) {
  ...
  const result = await transaction(async (connection) => {
    ...
    // Automatically generate and assign 6-digit Unique User ID (e.g. 000018) at registration
    const uniqueId = String(userId).padStart(6, '0');
    await connection.execute(
      `UPDATE users SET unique_id = :uniqueId WHERE id = :userId`,
      { uniqueId, userId }
    );

    // Insert wallet and auto-assign numeric wallet_id (e.g. 000018)
