...
import { ArrowLeft, Gift, Trophy, Search } from 'lucide-react';
import { userApi, apiAssetUrl } from '@/lib/api';
import Loading from '@/app/loading';

export default function OwnGiftWallPage() {
  ...
  if (error) {
    return (
      <div className="p-8 text-center text-red-400 bg-[#070B18] min-h-screen flex items-center justify-center flex-col space-y-4">
        ...
        <Link href="/user/profile" className="px-4 py-2 rounded-xl bg-white/10 text-white font-semibold text-xs">
          Back to Profile
        </Link>
      </div>
    );
  ...
  const userAvatar = profile?.avatar
    ...
    : (user.photo ? (apiAssetUrl(user.photo) || user.photo) : '/female-logo.svg');
      ...
      <Container className="max-w-5xl relative z-10 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-white/5 pb-6">
          <div className="flex items-center gap-4">
            <Link
              href="/user/profile"
              className="w-10 h-10 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center hover:bg-white/10 transition shrink-0"
          ...
          <Card className="p-12 text-center bg-[#101827]/50 backdrop-blur-xl border border-white/5 rounded-3xl space-y-3">
            ...
            <p className="text-xs text-zinc-400 max-w-sm mx-auto">
              {searchQuery
                ...
                : 'Virtual gifts received from other users in chat will automatically appear here on your Gift Wall!'}
            </p>
          </Card>
        ) : (
